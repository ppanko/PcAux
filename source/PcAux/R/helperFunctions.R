...
procStatus <- function(stp) {
    
    sesh <- list(sessionInfo())
    os <- as.character(Sys.info()["sysname"])

    if(os != "Windows" & os != "Linux") {

        os <- unlist(sesh)[grep("macOS", unlist(sesh))]
    ...
    if(os == "Windows") {
        ...
        ##splits <- unlist(strsplit(cpu2, ",  "))
        ##average <- splits[grep("load average", splits)]
        
    } else if (os == "macOS") {

        lookFor <- list("top -l1|egrep 'CPU usage|PhysMem'",
                        "system_profiler SPHardwareDataType|egrep 'Processor|Cache'")
        
    } else stop("Sorry, can't snoop your OpenBSD/Solaris/TempleOS")

    if(stp == "start") sesh[[2]] <- rapply(lookFor, system, intern = TRUE)

    else sesh <- rapply(lookFor, system, intern = TRUE)
